...
for (const city of cities) {
  // Find panels by checking their original full name or short name mapping
  // Since we already ran organize.js, they have short names
  const shortNamesMap = {
  ...
  const ubiPanel = Object.values(expanseData.objects).find(o => o.name === `UI Ubicación ${shortName}` || o.name === `UI Carrera Blanca`)
  const rumorPanel = Object.values(expanseData.objects).find(o => o.name === `UI Rumor ${shortName}` || o.name === `UI Rumor Carrera Blanca`)
  ...
  if (rumorPanel && city.rumors && city.rumors.length > 0) {
    ...
    if (btn1) wipeActionComponents(btn1)
    if (btn2) wipeActionComponents(btn2)
    if (btn3) wipeActionComponents(btn3)
...
console.log('Successfully wiped and reapplied strictly exact components!')
